...
def heart_prediction(input_data):
    ...
    if (prediction[0] == 0):
      return 'The person is not likely to have a Heart Disease'
    else:
...
def parkinson_prediction(input_data):
    ...
    if (prediction[0] == 0):
      return "The Person is not likely to have Parkinsons Disease"
    else:
      return "The Person is likely to have Parkinsons Disease"
...
def liver_prediction(input_data):
    ...
    if (prediction[0] == 0):
      return "The Person is not likely to have Liver issues"
    else:
      return "The Person is likely to have Liver issues"
...
def main():
        ...
        with st.sidebar:
            image_2 = Image.open("Baroda Institute of Technology Logo.png")
            st.image(image_2,width =200)
            image_1 = Image.open("nav_pic.png")
            st.image(image_1,width =215) 
            # st.title("Disease Diagnosis and Recommendation System")
        
            if st.button('🏠 Home',use_container_width=True):
                st.session_state.page = 'Home'
            if st.button('🩸 Diabetes Prediction',use_container_width=True):
                st.session_state.page = 'Diabetes'
            if st.button('🫀 Heart Disease Prediction',use_container_width=True):
                st.session_state.page = 'Heart'
            if st.button('🧬 Parkinsons Prediction',use_container_width=True):
                st.session_state.page = 'Parkinsons'
            if st.button('🩺 Liver Functioning',use_container_width=True):
                st.session_state.page = 'Liver'
            if st.button('⚖️ BMI CALCULATOR',use_container_width=True):
                st.session_state.page = 'BMI'
            # st.markdown(f"<p style='color: gray; font-size: 12px;'>Current Page: <b>{st.session_state.page}</b></p>", unsafe_allow_html=True)
            
        if st.session_state.page == 'Home':
            st.title("Medical Diagnosis Support Sytem")
            image = Image.open("front_pic.png")
            st.image(image, width=630)
            st.subheader("ℹ️ About This Project")
            st.markdown("""
            This project is developed using Python and Streamlit.  
            It leverages pre-trained machine learning models for disease prediction based on available datasets.  
            """)
        ...
        if st.session_state.page == 'Diabetes':
            ...
            if st.button('Diabetes predict'):
                if bool(Pregnancies)==False:
                    ...
                    diabetes_diagnosis = diabetes_prediction([float(Pregnancies),float(Glucose), float(Bp), float(SkinThickness),float(Insulin), 
                                                              float(BMI), float(DiaPedi),float(Age)])
        ...
        if st.session_state.page == 'Heart':
            ...
            fbs = col1.selectbox('Fasting Blood Sugar > 120 mg/dl',['Yes','No'])
            ...
            if st.button('Predict'):
                if bool(age)==False:
                    ...
                    heart_diagnosis = heart_prediction([float(age),float(sex_numeric),float(cp),float(trestbps),float(chol),float(fbs_numeric), 
                                                        float(restecg),float(thalach),float(exang), float(oldpeak),float(slope),float(ca),float(thal)])
                    st.success(heart_diagnosis)

        if st.session_state.page == 'Parkinsons':
            ...
            if st.button('Predict'):
                if bool(fo)==False:
                    ...
                    parkinson_diagnosis = parkinson_prediction([float(fo),float(fhi), float(flo), float(Jitter_percent),float(Jitter_percent), 
        ...
        if st.session_state.page == 'Liver':
            ...
            if st.button('Predict'):
                if bool(age)==False:
                    ...
                    liver_diagnosis = liver_prediction([float(age),float(sex_numeric), float(Total_Bilirubin), float(Direct_Bilirubin),float(Alkaline_Phosphotase), 
                                                        float(Alamine_Aminotransferase), float(Aspartate_Aminotransferase),float(Total_Protiens),float(Albumin),
